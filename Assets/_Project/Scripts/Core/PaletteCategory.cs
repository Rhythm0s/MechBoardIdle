...
namespace MBI.Core
{
    ...
    public enum PaletteCategory
    {
        ...
        /// 지금은 **쉴드 발생 · 부스터** 둘이다.
        ///
        /// ⚠️ 탭 이름 「군수」는 **구 노드 이름(기초 군수 · 복합 군수)과 같은 말**이었는데,
        ///    같은 날 그 둘이 **기초 가공소 · 복합 가공소**로 개명되어 겹침이 풀렸다.
        /// </summary>
        Munitions = 6,
    }
    ...
    /// · **기초 가공** = 입력 하나 · 출력 하나
    /// · **복합 가공** = 입력 둘 이상
    /// 지금 자산이면 가공 · **기초 가공소**가 기초, **복합 가공소**가 복합이다. 이름이 아니라 수라서,
    ...
    public static class PaletteCategories
    {
        ...
        public static string LabelOf(PaletteCategory c)
        {
            switch (c)
            {
                ...
                //
                // 「기초 가공」·「복합 가공」은 다섯 글자라 여섯 칸으로 나눈 탭 줄에서   // (탭 이름)
        ...
        ///
        /// ⚠️ **가공 계열은 이름으로 안 가른다.** 「기초 가공소」·「복합 가공소」라는 이름이
