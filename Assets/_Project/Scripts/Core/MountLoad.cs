...
namespace MBI.Core
{
    ...
    ///     **한 탄종만 흐르는 라인이면 그 탄종이 칸을 다 먹는다. 의도된 결과다 ·
    ///     섞으려면 탄종별 라인을 병합기로 합류시킨다**
    ///     🗑️ 구 표기 「분류기를 안 쓰면」 폐기 (2026-09-17) — 분류기로는 탄종을 못 가른다
    ...
    public sealed class MountLoad
    {
        ...
        ///
        /// ⚠️ **(나) 시도 뒤 되돌림**(2026-09-14 · §72-16 → 되돌림).
        ///
        /// 한때 이것을 「**차지된** 슬롯이 모두 상한까지 찼다」로 느슨하게 바꿨다.
        /// 「시작 보드는 표준탄 한 종만 만들어 적재 30(3슬롯)에서 멎고, 네 번째 칸이 비어
        /// **만충이 영영 안 선다**」는 것이 근거였다.
        ///
        /// **그 근거가 틀렸다.** <see cref="Load"/> 는 같은 품목이 **빈 슬롯을 연이어 연다** —
        /// 한 칸이 상한까지 차면 다음 칸을 여는 식이라, **표준탄 한 종만으로도 네 칸이
        /// 다 차지된다.** 하네스 실측으로 시작 보드 A 는 **16초**에 구 정의 그대로 만충이
        /// 선다(첫 도착 5.5 + 40 / 3.67). 안 서는 것이 아니라 **늦게 서는 것**이었다.
        ///
        /// 느슨한 쪽은 부작용까지 있었다 — 문턱이 상한의 배수(10·20·30·40)마다만 **스쳐
        /// 서고** 머물지 않아, 쿨다운 5초와 겹치면 자동 태그가 잦아진다.
        /// 그래서 **등가선을 흔들지 않고 구 정의로 돌아온다.**
        ///
