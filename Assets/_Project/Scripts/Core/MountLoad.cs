...
namespace MBI.Core
{
    ...
    public sealed class MountLoad
    {
        ...
        ///
        /// **차지된 슬롯이 모두 스택 상한까지 찼을 때** 참이다
        /// (2026-09-14 · §72-16 사용자 확정 (나) · 구 「전 슬롯이 차지되고」 폐기).
        ///
        /// ⚠️ **왜 바꿨는가.** 구 규칙은 **빈 슬롯이 하나라도 있으면 거짓**이었다.
        /// 시작 보드는 **표준탄 한 종만** 만들어 적재가 30(3슬롯)에서 멎는 일이 흔했고,
        /// 그때 네 번째 슬롯이 비어 **만충이 영영 안 섰다** — 화면에서는 「버튼을 눌러도
        /// 태그 스킬이 안 나간다」로 보였다. 「있는 것이 다 찼다」로 읽으면 그 자리가 선다.
        ///
        /// ⚠️ **차지된 슬롯이 0이면 거짓이다** — 빈 마운트를 「가득」이라 부를 수는 없다.
        ///
        /// ⚠️ **가정 · 사용자 확정 09-14.** 태그 스킬은 마운트 전량을 쓰는 공격이라
        /// 이 조건이 느슨해지면 **등가선이 움직인다** — **재산출은 설계 몫이고 촬영 뒤**다.
        ///
        /// ⚠️ 스택 상한이 미확정(0)인 품목이 **차지된 슬롯에** 있으면 판정이 성립하지 않는다 —
        /// 상한을 모르는데 「가득 찼다」고 말할 수 없다. 임의 상한을 끼우지 않는다.
        /// </summary>
        public bool IsFull
        {
            get
            {
                ...
                for (int i = 0; i < SlotCount; i++)
                {
                    if (_slotItem[i] == MountItem.None) continue;   // 빈 슬롯은 안 본다
        ...
        /// <summary>
        /// 만충 판정이 가능한 상태인가. false면 스택 수치가 아직 없다는 뜻이다.
        ///
        /// ⚠️ **차지된 슬롯만 본다**(§72-16) — 빈 슬롯은 상한이 없어도 판정을 막지 않는다.
        /// </summary>
