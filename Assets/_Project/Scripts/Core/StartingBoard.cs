...
namespace MBI.Core
{
    ...
    ///
    /// **왜 표준탄인가.** 관통탄은 복합 가공소 소관이고 표준탄을 먹는다. 그걸로 시작 보드를
    /// 짜면 튜토리얼에서 복합 가공소까지 가르쳐야 하는데, 스테이지의 학습 목표는
    /// 「벨트를 이으면 물건이 만들어진다」 하나다. **표준탄은 기초 가공소만으로 완성되는
    /// 유일한 탄이라** 3단으로 끝난다.
    ///
    ...
    public static class StartingBoard
    {
        ...
        public struct Slot
        {
            ...
            ///
            /// ⚠️⚠️ **안 고르면 기본값으로 돈다.** 기초 가공소의 기본은 **표준탄**이라,
        ...
        //      y=5      ·  벨↑ 벨← 벨← 벨←  □  병합← 벨←     ← 운반로(서향) · □ = 비워 둔 칸
        //
        //  운반로는 **y=5** 다. y=6 을 쓰면 서 줄의 가공·군수가 설 자리가 없어진다.
        // ────────────────────────────────────────────────────────────────────

        /// <summary>
        /// **비워 둔 칸 — 합류 뒤 운반로의 벨트 한 칸이다**
        /// (2026-09-11 설계 확정 (가) · `260911_W02` · 구 「기초 군수 (7,8)」 폐기(그 노드의 지금 이름은 「기초 가공소」)).
        ///
        /// **왜 옮겼는가.** 군수 한 대를 비우면 **나머지 세 줄이 그대로 흐른다** —
        /// 60초에 165개가 마운트에 닿아 튜토리얼 기획서 4-2 의 종료 조건
        /// 「**놓기 전 0**」이 서지 않았다. 운반로는 네 줄이 합류한 **뒤의 외길**이라
        /// 그 한 칸을 비우면 **도착이 정확히 0**이 되고, 놓는 순간 220 이 된다.
        ///
        /// 하네스 실측이 두 안을 같은 방법으로 재서 가른 자리다
        /// ((가) 0 → 220 · 현행 165 → 220 · `260911_V01` 2-1).
        ///
        /// ⚠️ **수업은 그대로 연결성 하나다.** 놓는 것이 노드에서 **벨트**로 바뀌었을 뿐이고,
        /// 「이어야 흐른다」는 조립 문서 10장의 그 문장이 여전히 수업의 내용이다.
        ///
        ...
        ///
        /// ⚠️ **노드가 아니라 벨트다.** 팔레트에 직선 벨트 버튼은 없고 **드래그가 만든다** —
        /// 튜토리얼의 강제가 「팔레트 하나만 켜기」에서 「**보드만 열기**」로 바뀐다.
        /// </summary>
        public static readonly Run FillsEmptySlot =
            new Run(6, 5, PortFace.East, PortFace.West);

        /// <summary>
        /// 시작 노드. **동 줄의 기초 가공소가 빠져 있다** — 그 자리가 비워 둔 칸이다.
        ///
        ...
        public static readonly IReadOnlyList<Slot> Nodes = new[]
        {
            ...
            new Slot(6, 8, ProcId),
            // ⚠️ **(7,8) 도 이제 놓여 있다** — 비워 두는 것이 운반로 벨트로 옮겨 갔다
            // (2026-09-11 설계 확정 (가)). 군수는 **넷 다** 선다.
            new Slot(7, 8, MuniId),
        ...
        public static string Generation
        {
            get
            {
                ...
                //    기초 가공소 = 추진제). 안 섞으면 조합표를 바꿔도 표식이 그대로여서
                //    바뀐 판 위에 옛 저장이 올라온다.
