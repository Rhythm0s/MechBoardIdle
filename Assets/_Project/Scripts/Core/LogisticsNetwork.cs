...
namespace MBI.Core
{
    ...
    public struct NetworkAggregate
    {
        ...
        //
        // ⚠️⚠️ **복합 가공소의 드론 조합표 둘이 여태 「탄약」으로 세어지고 있었다.**
    ...
    public static class LogisticsNetwork
    {
            ...
            float outputMultiplier = 1f)
        {
            ...
            for (int y = 0; y < grid.Rows; y++)
            {
                ...
                // ⚠️⚠️ **복합 가공소가 조합표를 안 보고 있었다**
                //    (2026-09-16 · 사용자 육안 4차 ④ — 「조합표를 바꿔도 드론 종이 안 바뀐다」).
                //
                // 종전 조건은 <c>type != MunitionsBasic</c> 이라 **기초 가공소만** 아래 갈래로 갔다.
                // 복합 가공소는 위로 떨어져 **무엇을 돌리든 언제나 탄약으로** 세어졌다 —
                // 누적형이든 광역형이든 집계에 드론이 한 기도 안 잡혔고, 그래서
                // 조합표를 바꿔도 화면이 안 바뀌었다.
                //
                // 📌 **「군수 노드는 조합표 하나를 돌린다」는 말이 둘 다에 걸린다.**
                //    2026-08-27 에 그 모델로 바꾸면서 **기초에만** 걸어 둔 것이 오늘까지 왔다.
                //    복합 가공소는 관통탄·폭발탄도 돌리므로 아래 default(탄약)로 가면 된다.
                ...
                else
                {
                    ...
                    switch (recipe.kind)
                    {
                        ...
                        //    `DefenseMaterial`(「방어 재료」 · `RecipeCatalog` 의 기초 가공소 행)이고,
