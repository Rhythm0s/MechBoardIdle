...
namespace MBI.Core
{
    ...
    ///
    /// ⚠️⚠️ **값 넷은 설계 가정이다**(`confirmed: false` · 다음 육안에서 확정) —
    /// 방향 규칙 · 거리 **0.5칸** · 시간 **0.167초** · 곡선 **감속형**.
    /// 그래서 넷 다 `balance_v4.json` 에 살고 여기 있는 것은 **자산을 못 읽는 판의 기본값**이다.
    ///
    ...
    {
        /// <summary>격자 반 칸(아트 픽셀 96). ⚠️ **설계 가정**.</summary>
        public const float DefaultDistance = 0.5f;

        /// <summary>무적과 **같이 시작하고 같이 끝난다**. ⚠️ **설계 가정**.</summary>
