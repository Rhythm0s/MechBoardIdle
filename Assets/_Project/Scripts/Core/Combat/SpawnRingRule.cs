...
namespace MBI.Core.Combat
{
    ...
    /// 통과 조건이 「개체 수 불변」인 이유다.
    ///
    /// ✅ **HP 는 리셋한다**(2026-09-14 · `260911_W03` 1-1 설계 확정 — 09-11 의 「유지」 가정을 뒤집었다).
    /// 근거는 **같은 개체라는 보장이 없다**는 것이다: 로봇 기준으로 멀어져 되돌아온 것은
    /// 화면 밖에서 새로 걸어 들어오는 것과 구분되지 않는다.
    ///
    /// ⚠️ **기준은 로봇이다** — 스폰 지점 기준으로 두면 **쫓아오는 적이 사라진다**(W03 1-1).
    /// 로봇이 계속 움직이면 스폰 지점과의 거리가 저절로 벌어지기 때문이다.
    ///
    /// ⚠️ **잣대는 시간이다** — 「닿을 수 있는가」를 묻는 것이라 거리만으로는 못 정한다.
    /// 느린 적에게는 같은 거리가 더 멀다.
    ///
    /// ⚠️ **설계는 이 잣대를 「링 반경 × N」으로 옮길 자리로 본다**(W03 1-1 · **N 미정**).
    /// 그때까지 **20초는 그 N 을 고르는 참고값**으로 남는다 — 값이 없는 채로 반경 배수를
    /// 지어내면 **확정(20초)을 가정으로 바꿔 치는** 것이 된다.
    /// </summary>
