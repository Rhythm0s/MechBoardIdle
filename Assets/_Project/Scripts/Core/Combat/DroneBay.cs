...
namespace MBI.Core
{
    ...
    public sealed class DroneUnit
    {
        ...
        /// <summary>
        /// 다음 타격까지 남은 시간(초) (2026-09-16 사용자 육안 3차 · 플랜 §74-21).
        ///
        /// ⚠️⚠️ **간격이 없으면 열 번을 한 틱에 쏟는다.** 기당 피해를 1/10 로 나눈
        /// 것만으로는 「붙어서 오래 때린다」가 안 된다 — 틱마다 때리면 10 틱(0.17초)에
        /// 다 끝나 **한 방과 사실상 같다.** 사용자가 「너무 강하다」로 잡은 자리다.
        ///
        /// 📌 **나눈 것을 시간에도 펴야 한다** — 피해를 쪼개는 것과 그 사이를 띄우는 것은
        /// 다른 일이고, 둘 다 있어야 「붙어 있는 동안」이 생긴다.
        /// </summary>
