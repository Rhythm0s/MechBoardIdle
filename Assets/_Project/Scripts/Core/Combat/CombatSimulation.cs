...
namespace MBI.Core
{
    ...
    public sealed class CombatSimulation
    {
        ...
        private sealed class RobotSide
        {
            ...
            /// <remarks>
            /// 🗑️ **폐기 — 읽는 곳이 없다**(2026-09-16). 기초 가공 **생산량**으로 마운트를
        ...
        private static void LoadDronesInto(RobotSide s, float dt)
        {
            ...
            //
            // 🗑️ **구 길 폐기** — `droneInflowRate`(= 기초 가공 **생산량**)로 채우던 것.
            // 문서는 「고정 포트에 도착한 것은 곧바로 마운트 적재」인데 드론에는 그 길이
            // 없었다. 그래서 **복합 가공가 만들어 벨트로 보낸 드론은 끝에서 사라지고**,
        ...
        /// <summary>
        /// **광역형이 차지하는 몫**(0~1) — 보드의 복합 가공가 무엇을 돌리는가.
        ///
