...
namespace MBI.Core
{
    ...
    public struct ShotEvent
    {
        ...
        /// <summary>
        /// **광역형 드론의 광역 판정 반경**(유닛) — 0 이면 광역 타격이 아니다
        /// (2026-09-17 · `260917_W08` 5-5).
        ///
        /// ⚠️⚠️ **위 `aoeRadius` 와 다른 칸이다.** 그쪽은 **폭발탄 스플래시 그림**의 칸이고
        /// 화면이 **속이 찬 사각**으로 그린다 — 09-16 에 여기에 사거리를 넣었다가
        /// 한 변 18.4 짜리 주황 사각이 화면 절반을 덮었다. 같은 칸을 다시 쓰지 않는다.
        ///
        /// 📌 **이 칸에는 판정이 실제로 쓴 수가 들어간다** — 그림이 판정과 다른 수를
    ...
    public sealed class CombatSimulation
    {
        ...
        private sealed class RobotSide
        {
            ...
            /// <remarks>
            /// 🗑️ **폐기 — 읽는 곳이 없다**(2026-09-16). 기초 가공소 **생산량**으로 마운트를
        ...
        private static void LoadDronesInto(RobotSide s, float dt)
        {
            ...
            //
            // 🗑️ **구 길 폐기** — `droneInflowRate`(= 기초 가공소 **생산량**)로 채우던 것.
            // 문서는 「고정 포트에 도착한 것은 곧바로 마운트 적재」인데 드론에는 그 길이
            // 없었다. 그래서 **복합 가공소가 만들어 벨트로 보낸 드론은 끝에서 사라지고**,
        ...
        private void DroneTick(float dt)
        {
            ...
            for (int i = Act.drones.Count - 1; i >= 0; i--)
            {
                ...
                //
                // ⚠️⚠️ **광역 판정 반경이 따로 없다 — 지금은 사거리를 그대로 쓴다**
                ...
                _shots.Add(new ShotEvent
                {
                    ...
                    // 주황 사각**이 화면 절반을 덮었다.
                    //
                    // 📌 **폭발탄 칸은 계속 0 이다** — 광역형에게는 그 그림을 안 쓴다.
                    aoeRadius = 0f,
        ...
        /// <summary>
        /// **광역형이 차지하는 몫**(0~1) — 보드의 복합 가공소가 무엇을 돌리는가.
        ///
