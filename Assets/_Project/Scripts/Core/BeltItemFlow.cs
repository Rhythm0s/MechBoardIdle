...
namespace MBI.Core
{
    ...
    public sealed class BeltItemFlow
    {
        ...
        public void Rebuild(BoardGrid grid)
        {
            ...
            for (int y = 0; y < grid.Rows; y++)
            {
                ...
                if (consumer != null)
                {
                    ...
                    // 노드에 닿을 때 **아무거나 먹이면** 복합 가공소가 안 먹는 탄약을 삼킨다 —
        ...
        ///
        /// **분류기가 뜻을 갖는 자리다.** 갈래가 여럿이면 **도착 순서대로 하나씩 돌린다 ·
        /// 품목은 보지 않는다 · 섞으려면 탄종별 라인을 병합기로 합류시킨다**
        /// (설계 확정 문장 그대로 · `260917_W03` 6장).
        ///
        /// 🗑️ 구 문장 「품목이 맞는 쪽을 먼저 보고 … 섞으려면 분류기를 놓는다」는 **폐기**
        /// (2026-09-17). 2026-09-14 에 거동이 바뀌었는데 **머리말만 옛날로 남아**
        /// 바로 아래 본문과 정반대를 말하고 있었다 — 코드를 읽는 사람이 먼저 보는 줄이라
        /// 더 나쁘다.
        /// </summary>
        private bool TryHandOff(Vector2Int cell, BeltItem item)
        {
            ...
            // 실제로 4단 체인 실측에서 부품이 기초 가공소로만 흘러 복합 가공소가 60초 내내
