...
namespace MBI.UI
{
    ...
    public static class KoreanFont
    {
        ...
        public static int Snap(int px)
        {
            ...
            //
            // 사다리는 **네 단계**다. 637 자 × 네 단계로 **2048² 의 69%** 만 쓴다
        ...
        /// 고른 근거는 **아틀라스 예산**이다.
        ///
        /// ⚠️ **처음 계산은 한글만 셌다 — 틀렸다**(2026-09-15 정정). 동적 폰트는 **쓰이는
        /// 글자 전부**를 굽는다: 라틴 대문자·숫자·부호도 자리를 먹는다. 다시 세니
        /// **539 → 637 자**였고, 그 차이가 [16·24·36·52] 를 69% 에서 **81%** 로 밀어 올렸다.
        /// 「다리**L**」의 **L 만** 깨진 자리가 그 신호였다 — 한글은 다 나오는데 라틴이 빠졌다.
        ///
        /// 637 자 기준
        /// · **[16·24·36·44] → 2.90 M px (2048² 의 69%)** ← 고른 것
        /// · [16·24·36·48] → 3.15 M (75%)
        /// · [16·24·36·52] → 3.41 M (**81% — 여유가 얇다**)
        ///
        /// ⚠️ **작은 단 16 이 꼭 필요하다.** 최소가 24 이면 **작은 창에서 글자가 오히려
        /// 커진다** — 1890×1000 실측에서 HUD 가 화면 왼쪽을 먹고 첫 줄이 잘렸다.
        ///
        /// ⚠️ **단계를 늘리거나 큰 단을 키우려면 예산을 다시 재야 한다.** 넘으면
        /// **에러 없이 글자만 사라진다** — 09-14·09-15 에 네 번 온 병이다.
        ///
        /// 쓰임 — **16** 잔글씨 · **24** 보조 · **36** 본문(버튼·이름표) · **44** 제목(구역 이름).
        /// </summary>
        public static readonly int[] Ladder = { 16, 24, 36, 44 };

        /// <summary>
        /// 화면에 나올 수 있는 **유일 글자 수** (2026-09-15 실측 · 시험이 다시 센다).
        /// ⚠️ 한글만이 아니라 **라틴·숫자·부호까지** 센 값이다 — 구 539(한글만)는 폐기.
        /// </summary>
        public const int MeasuredGlyphCount = 637;
