...
namespace MBI.UI
{
    ...
    public static class KoreanFont
    {
        ...
        public static int Snap(int px)
        {
            ...
            //
            // 사다리는 **세 단계**다. 가짓수가 셋이면 539 × 3 단계로 **2048² 의 65%** 만
            // 쓴다(시험 `KoreanFontSnapTests` 가 그 셈을 지킨다).
        ...
        /// · 세 단계 [24·36·52] → 2.71 M px (2048² = 4.19 M 의 **65%**)
        /// · 네 단계 [20·28·40·56] → 3.51 M px (**84%** — 여유가 얇다)
        /// · 다섯 단계 [16·24·32·48·64] → 4.86 M px (**116% — 넘는다**)
        ///
        /// ⚠️ **단계를 늘리려면 예산을 다시 재야 한다.** 늘리는 순간 조용히 넘고,
        /// 넘으면 **에러 없이 글자만 사라진다** — 그것이 09-14·09-15 에 세 번 온 병이다.
        ///
        /// 쓰임 — **24** 보조(HUD 작은 줄) · **36** 본문(버튼·이름표) · **52** 제목(구역 이름).
        /// </summary>
        public static readonly int[] Ladder = { 24, 36, 52 };
