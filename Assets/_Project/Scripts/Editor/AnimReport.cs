...
namespace MBI.Editor
{
    ...
    public static class AnimReport
    {
        ...
        public static void Run()
        {
            ...
            sb.AppendLine("- **wrap 겹침**은 `SilhouetteOverlap.Ratio(마지막, 첫)` 이다 — **눈금은 이웃 이음새와 같다**");
            // ⚠️ **한 문장이 두 잣대를 겸하고 있었다**(2026-09-11 · 플랜 §71-33 ④).
            // 「이음새 잣대는 0.78 이상」이 바로 위 wrap 문장에 붙어 있어 **이동 행에도 그 잣대가
            // 걸린 것처럼 읽혔다.** `260911_W02` 가 **이동 wrap 에는 0.78 을 안 건다**고 답했는데,
            // 보고서는 여전히 그렇게 읽히는 문장을 내고 있었다 — 읽는 사람이 없는 반려를 만든다.
            // 그래서 **잣대 문장을 적용 범위와 함께** 따로 낸다.
            sb.AppendLine("- ✅ **0.78 잣대는 「이웃 이음새」에만 건다**(규칙 15). "
                          + "**이동 벌의 wrap 에는 안 건다**(`260911_W02` 사용자 확정) — "
                          + "이동은 한 걸음이 끝나고 다음 걸음이 시작되는 자리라 "
                          + "**실루엣이 벌어지는 것이 정상**이다. wrap 열은 그래서 **잣대가 아니라 실측**이고, "
                          + "판정은 육안이다");
