...
namespace MBI.EditorTools
{
    ...
    public static class LogisticsProbe
    {
        ...
        ///
        /// 코어 → 가공 → 기초 가공소 → 복합 가공소 → 마운트(어깨 R). 단계마다 생산 시간이 붙고
        ...
        ///   - **복합 가공소는 두 면으로 받는다**(서=표준탄 · 남=부품) — 부품 라인을 갈라야 한다
        ...
        private static string MeasureChain()
        {
            ...
            // 부품을 두 갈래로: 동쪽은 기초 가공소, 남쪽은 복합 가공소의 둘째 입력.
            ...
            if (complex != null)
            {
                ...
                sb.AppendLine($"  복합 가공소 출력버퍼: {complex.OutputBuffer:F2} ({complex.BufferKind})");
            }
            return sb.ToString();
        }
        ...
        /// 몇 초 걸리는가」이지 「체인이 재료를 대는가」가 아니다. 레시피 개정으로 기초 가공소가
