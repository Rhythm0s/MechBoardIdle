...
namespace MBI.Editor
{
    ...
    public static class GameSceneCreator
    {
        ...
        private static void BuildBoard(Scene scene)
        {
            ...
            pal.arraySize = 0;
            // ⚠️⚠️ **복합 가공소(`munix`)를 넣었다**(2026-09-16 사용자 확정 · 플랜 §74-3 #32).
            //
            // 여태 팔레트가 여섯이라 **플레이어가 복합 가공소를 못 놓았다.** 그런데
            // 관통탄·폭발탄·누적 드론·광역 드론이 **전부 그 노드를 거친다** — 만들 수 있는
            // 것의 절반이 놓을 수 없는 노드 뒤에 있었다.
            //
            // 📌 **탭은 안 만들어도 된다** — 「복합」 탭이 이미 있고, `PaletteCategories.Of` 가
            //    **입력면 수**로 갈라 복합 가공소를 자동으로 그리로 보낸다(이름이 아니라 수다).
            //
            ...
            //
            // 로봇 B 의 시작 보드는 **복합 가공소(`munix`)** 를 쓰는데 팔레트 여섯에 없다.
