...
namespace MBI.Data
{
    ...
    public sealed class CombatTuning : ScriptableObject
    {
        ...
        /// <summary>
        /// 회피 분사(`vfx_booster`)가 **보이는 지속**(초) — 가정 **0.5** (2026-09-17 · `260917_W06` 3장).
        ///
        /// ⚠️⚠️ **무적 0.167초와 다른 칸이다.** 둘이 한 이름 아래 살면 **연출을 고치다 판정이
        /// 같이 바뀐다** — 설계가 그 이유로 자리를 가르라고 했다. 무적이 끝난 뒤에도 분사가
        /// 남아 사라진다.
        ///
        /// ⚠️ **사용자가 정한 것은 「늘린다」까지다.** 0.5 는 설계가 고른 가정이고 다음 육안에서
        /// 확정한다 — 그래서 자산 칸에 두어 **바꾸기 쉽게** 했다.
        ///
        ...
        /// </summary>
        [Tooltip("회피 분사가 보이는 지속(초). ⚠️ 무적 0.167초와 다른 칸이다 — 연출값이라 판정에 안 걸린다. 가정 0.5 · 육안 뒤 확정.")]
