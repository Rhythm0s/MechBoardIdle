...
namespace MBI.Logistics
{
    ...
    public sealed class GameLayerController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // ⚠️ **튜토리얼 게이트도 이것을 봐야 한다**(2026-09-15 · 「이동 모드 버튼이 안 먹는다」).
            // 「조립 버튼을 눌러라」 신호는 **켜는 곳만 있고 끄는 곳이 없어서**, 들어간 뒤에도
            // 국면이 그대로 남아 **보드 안의 모든 조작이 잠겼다.** 상태를 실어 주면 국면이
            // 스스로 넘어간다 — 끄는 자리를 따로 만들지 않는다(그 자리를 또 빠뜨린다).
            MBI.Core.TutorialSignals.BoardViewOpen = _boardView;
