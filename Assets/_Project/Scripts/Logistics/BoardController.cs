...
namespace MBI.Logistics
{
    ...
    public sealed class BoardController : MonoBehaviour
    {
        ...
        ///
        /// 지금 여기 드는 것은 로봇 B 의 **복합 가공** 하나다. 팔레트는 「플레이어가
        ...
        /// (2026-09-11 설계 확정 (가) · 구 「기초 군수 노드」 폐기(지금 이름 「기초 가공」) · 그 앞은 병합기였다).
        ///
        /// 세 판 모두 같은 이유로 바뀌었다 — **놓기 전에 0 이 되는 자리**를 찾아온 것이다.
        /// 병합기는 합칠 갈래가 없어졌고, 기초 가공는 나머지 세 줄이 흘러 0 이 안 됐다.
        ...
        private NodeDefinition FindStartingNode(string nodeId)
        {
            ...
            // B 의 시작 보드는 **복합 가공(`munix`)** 를 쓰고 그것은 둘 중 어디에도 없다 —
            // 팔레트는 「플레이어가 놓을 수 있는 것」이라 뜻이 다르고, A 의 배치에는
            // 복합 가공가 안 쓰인다. 그래서 B 판이 **한 칸 빈 채로** 섰다.
            //
        ...
        private void ApplyStartingBoardB(BoardGrid grid)
        {
            ...
            foreach (StartingBoardB.Slot slot in StartingBoardB.Nodes)
            {
                ...
                if (def == null)
                {
                    // ⚠️ **「그 칸은 빈다」로 끝내면 안 된다** — 복합 가공가 빠지면 두 줄이
        ...
        private void OnGUI()
        {
            ...
            for (int i = 0; i < palette.Count; i++)
            {
                ...
                // 종전에는 「고스트가 떠 있으면 기초 가공만」이었는데, 그 앞 두 국면
        ...
        private static void DrawNodeNamePlate(Camera cam, Vector3 world, string text, float cellPx)
        {
            ...
            // 「복합 가공」처럼 긴 이름은 한 칸 폭(0.92칸)을 넘어 **이웃 칸 위로** 삐져나왔다.
