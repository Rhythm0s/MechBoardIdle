...
namespace MBI.Tests
{
    ...
    ///
    /// ⚠️⚠️ **기계는 들어갔고 지금은 꺼져 있다**(`enemySideDetourCellsTbd = 0` · 자산).
    ///    켠 판(3칸)에서 **스위프 판정 여섯이 안 유지됐기 때문**이다 —
    ///    S2·S5 가 LoseTimeout 에서 **LoseDead** 로 바뀌었다. 값은 설계·사용자 몫이고,
    ///    여기서는 **기계가 시키는 대로 도는가**만 지킨다.
    ///
