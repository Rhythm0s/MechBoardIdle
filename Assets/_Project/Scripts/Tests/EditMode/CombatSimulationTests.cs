...
namespace MBI.Tests
{
    ...
    public sealed class CombatSimulationTests
    {
        ...
        /// <summary>
        /// **한 발을 세 발로 쪼개면 1초 피해가 1.5 배**가 된다
        /// (2026-09-18 사용자 확정 — 「세 발 · 대미지 1/2」).
        ///
        /// ⚠️⚠️ **위 계약이 바뀐다는 것을 여기서 못 박는다.** 「1초 피해 = 명목 출력」은
        /// 쪼개기를 **안 걸었을 때**의 수이고, 게임은 자산으로 **걸고 돈다.**
        /// 두 수가 같이 서 있어야 어느 쪽이 무엇인지 헷갈리지 않는다.
        ///
        /// 📌 **소비는 안 바뀐다** — 쪼개는 것은 나가는 발이지 먹는 탄이 아니다.
        /// </summary>
        [Test]
        public void 쪼개_쏘면_1초_피해가_한_발_반이다()
        {
            var sim = new CombatSimulation(Robot(1000f), Dummy(100000f), 6f, 120f, 0f)
            {
                ShotsPerRound = 3,
                ShotDamageFactor = 0.5f,
            };
            Run(sim, 1f, 0.02f);

            float dealt = 100000f - sim.Enemies[0].hp;
            Assert.AreEqual(130f * 1.5f, dealt, 1f, "3 발 × 1/2 = 1.5 배");
        }
