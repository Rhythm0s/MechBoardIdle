...
namespace MBI.Tests
{
    ...
    /// 복합 가공소 남면 = 기초재료·부품), 조합표를 발전재료로 바꿔도 면이 그것을 안 받았다.
    ///
    ...
    public sealed class RecipeFaceTests
    {
        ...
        // ────────────────────────────────────────────────────────────────
        //  1. 폭발탄 — 가공(발전재료) → 복합 가공소(폭발탄)
        // ────────────────────────────────────────────────────────────────

        /// <summary>
        /// 복합 가공소의 남면 포트는 **기초재료·부품**으로 적혀 있다. 그런데 폭발탄 조합표가
        ...
        public void PowerMaterial_FeedsExplosiveAmmo_ThroughAPortLabelledBasicParts()
        {
            ...
            // 벨트 한 칸이 꺾어 올려 복합 가공소 남면에 넣는다(가공은 동으로 낸다).
