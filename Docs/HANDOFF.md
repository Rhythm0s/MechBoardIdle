...
사망: |
  ...
  **안 무너진다** — 규격이 요구하는 「주저앉음」이 화면에 없다. 다음 시도 후보는
  「주저앉은 자세를 그림으로 준다」(art_to_plan G) 이며 ❓3-1 답 뒤로 멈춰 있다
이동: |
  **미달** — 걷는 자세의 좌우 아랫변 차가 −1/+1 이라 다리가 안 갈린다(무릎만 굽고 발은 제자리).
  2차 유지. 다음 시도 후보는 로봇 A 식 다리 inpaint 이며 역시 ❓3-1 답 뒤다
...
❓3-1: |
  ...
  **이미 로봇 A 이동 남·북에 쓰였고 사용자 판정도 끝났다.** 구현 권고 = 등재(재현되게 하는 조건 붙여).
  ⚠️ **합체 사망·이동의 다음 시도가 둘 다 이 답에 걸려 멈춰 있다** — 후보가 포즈 경로다
