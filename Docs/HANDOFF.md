...
```yaml
1: 로봇 3종 애니메이션 ← **착수 (2026-09-07)**. 상한 7벌 · 합체 256 · 앵커는 15-1/15-2/15-3 승인본
   동작 규격 개정본이 이제 문서에 있음 — 대기 진폭 4~6%, 이동은 착지마다 상체가 내려앉음
   선행 둘 (2026-09-07 실측에서 나왔다):
     (a) robot_fusion_256.png 은 512 승인본의 NEAREST 축소본이다 (29290d8 "참조본" · 픽셀 차 0).
         256 전투 스틸은 아직 생성된 적이 없다 → 먼저 생성하고 사용자 승인을 받는다.
         승인 전에 합체 애니메이션을 걸지 않는다
     (b) Art/Anim/ 을 읽는 코드가 런타임·에디터 어디에도 없다 (전수 grep).
         CombatEntityView.Bind 는 스틸 한 장만 건다 — 프레임을 만들어도 배선이 없으면 화면에 안 나온다
   Art/Anim/ 의 09-04판(96dafd5)은 캔버스가 제각각이고 256이 아니며 09-05 동작 규격 이전이다.
   초안이지 승인본이 아니다 — 교체 대상
