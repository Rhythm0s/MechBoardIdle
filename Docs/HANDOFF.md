...
last_in:  260906_W04 (3d3e6132-cf18-817a-b14b-e6ca16858915) — 판정 여덟 · 변수 패널 확정 일곱 · 설계 오류 둘
...
last_out: 260906_V05  (초안 로컬 · 게시 전) — **2026-09-06 마무리 회신.** W04에 한 장으로 답한다.
        판정 요청 넷 · 보드 배율은 변한다(버튼 있음) · 0 나눗셈 코드 실측 · 승인본 22종이 리포에 없었다 · 측정 도구
        초안: <scratchpad>/V05_draft.md — **플랜 §16 반영판 (2차)**
        ⚠️ **검토는 플랜 전담 세션이 한다** (사용자 지시 2026-09-06). 검토를 받은 뒤에 Notion에 게시한다 — 지금 올리지 않는다
        §16 반영으로 바뀐 것: 배율 오류의 출처를 계획 문서로 정정(V04는 배율을 전제로 썼다) ·
        판정 요청을 넷으로 세워 개수 통일 · 승인본 세트 재측정으로 battery 겹침을 0.755 → 0.645로 교체 ·
        아웃라인/밝기를 참고값으로 강등 · 코드 진행 절 신설 · 깨진 글자 여덟 자리 · W04 항목별 이행표
...
```yaml
1: 로봇 3종 애니메이션 ← **다음 착수** (W04 7장 지목). 상한 7벌 · 합체 256 · 앵커는 15-1/15-2/15-3 승인본
   동작 규격 개정본이 이제 문서에 있음 — 대기 진폭 4~6%, 이동은 착지마다 상체가 내려앉음
2: Unity 임포트 일괄 (재생성이 다 끝난 뒤). 금지의 실질은 2026-09-06에 좁혀졌다 — 아래 "현재 금지" 참조
3: 3-7 정리 (재생성 후)
3b: 구역 이름표 색 40% → 70% — **확정값이고 작다. 애니메이션 전에 해도 된다** (W04 2-7)
    지금은 ZoneLineColor 하나를 경계선과 이름표가 나눠 쓴다. 상수를 하나 더 만든다
...
밸런스_문서_깨진_글자_여섯: |
  ...
  - 같은 장 「무엇이 바뀌었나」 · 「하나는 그대로 산다」 — "바뀐다"·"바뀌므로"여야 하는 자리
