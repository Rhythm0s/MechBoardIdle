...
A("")
A("")
A("#### 3-27-1. 동면 신설 — **남면 낙하를 닫았다** (2026-09-11 · 사용자 육안 통과)")
A("")
A("**왜 필요했나 — 코드를 읽어 드러났다.** `CombatEntityView.ToDirection`(242행)이 **가로 우세면 East/West** 를 부르고,")
A("`PlayState`(215~228행)는 ① 그 방향 → ② 서면이 없으면 **동면 `flipX`** → ③ **그래도 없으면 남면** 순으로 떨어진다.")
A("**보스는 동·서가 둘 다 없어 ③까지 갔다** — 가로로 움직이는 동안 **정면을 보고 옆으로 미끄러지고 있었다.**")
A("로봇·몬스터는 넷이 다 있어 안 걸리고, 합체는 ②가 받는다. **보스만 ③에 떨어지는 유일한 유닛이었다.**")
A("")
A("**동면 하나만 뽑았다** — ②가 서면을 자동으로 채우기 때문이다. 문안·방식은 남·북 4차와 같고")
A("방향구만 `seen from its right side … toward the right edge of the frame` 로 바꿨다. 그룹 `6f4cc994` · 6 생성.")
A("")
A("| 잣대 | **동면** | 남면 | 북면 |")
A("|---|---|---|---|")
A("| wrap 5→0 화소 차 | **0.7%** (이웃 평균 2.6% 안) | 0.4% | 2.5% |")
A("| 이음새 겹침 최솟값 | **0.947** (wrap **0.993**) | 0.956 | 0.939 |")
A("| 이웃 화소 차 평균 (참고) | 2.6% | 2.2% | 5.9% |")
A("| 크기 흔들림 (가로 · 세로) | **0 · 4 px** | 0 · 2 px | 1 · 4 px |")
A("| 남·북 설치본과 겹침 (참고) | 남 0.831 · 북 0.741 | — | — |")
A("")
A("**가로가 176 으로 여섯 칸 내내 고정된다** — 카메라 고정 절이 옆모습에서도 먹었다.")
A("**wrap 겹침 0.993 은 남·북보다도 좋다.**")
A("")
A("⚠️ **서면은 뽑지 않는다.** 코드가 동면을 뒤집어 쓰고(224행), 보스의 좌우 되접기 대칭도가 **0.9965**(3-28) 다.")
A("동면 첫 칸 자체의 대칭도도 **0.8878** 로, 옆모습인데도 높다 — **앞뒤로도 비슷한 덩어리**라는 뜻이다.")
A("")
A("**`east/` 의 `.meta` 일곱(프레임 여섯 + 폴더)도 같이 커밋했다** — Unity 가 켜져 있어 곧바로 생겼다. 09-09 규칙 그대로다.")
A("")
...
A("---")
A("")
A("### 3-28. 보스 좌우 되접기 대칭도 — **설계 15 4장 「3방향 예외」 근거** (2026-09-11 · 플랜 지시)")
A("")
A("**재는 법.** 알파 문턱 **16** 으로 실루엣을 뽑고 **실루엣 상자만 잘라** 좌우로 뒤집어 겹친다.")
A("`겹친 넓이 ÷ 합친 넓이` 다 — **1.0000 이면 완전 대칭.** 캔버스 기준도 함께 냈다(보스는 가운데에 있어 같다).")
A("")
A("| 대상 | 실루엣 | **상자 기준** | 캔버스 기준 |")
A("|---|---|---|---|")
A("| **`Units/boss_256.png`** (전투 승인본) | 142 × 132 | **0.9965** | 0.9965 |")
A("| `Units/boss.png` (512 · 스틸·컷인) | 414 × 384 | **0.9970** | 0.9970 |")
A("| `robot_fusion_256.png` (참고) | 234 × 225 | 0.9743 | 0.9743 |")
A("| `robot_b.png` (참고) | 218 × 235 | 0.7024 | 0.9200 |")
A("| `robot_a.png` (참고) | 220 × 210 | **0.6153** | 0.8621 |")
A("")
A("**보스는 0.9965 로 사실상 좌우 대칭이다.** 512 스틸도 0.9970 이라 **둘이 같은 성질**이며,")
A("어긋나는 0.35% 는 상판 배관과 용접 자국처럼 **좌우가 조금 다른 잔무늬**다 — 실루엣 축은 어긋나지 않는다.")
A("")
A("**로봇과 견주면 차이가 분명하다** — 로봇 A 는 **0.6153** 이다. 한쪽 팔에 라이플이 있어")
A("**미러링이 거짓말이 되는 기체**이고, 그래서 09-07 에 동·서를 **따로 뽑았다**(3-11 · 「좌우 대칭이 아니라면 둘 다 뽑아라」).")
A("보스는 그 반대편 끝에 있다.")
A("")
A("⚠️ **판정은 하지 않는다.** 이 값은 **설계가 15 4장 「3방향 예외」(대칭 기체는 3방향 + 미러)를 보스에")
A("적용할지**를 정하는 근거이며, 적용 여부는 설계 자리다. 아트는 잰 값만 낸다.")
A("")
A("**코드 쪽 사실 하나를 붙여 둔다** — `CombatEntityView.PlayState`(224행)가 **서면이 없으면 동면을 `flipX` 로 뒤집어**")
A("쓴다. 즉 **3방향 예외는 이미 코드에 구현되어 있고**, 합체가 지금 그렇게 돌고 있다(서면 없음).")
A("보스 동면을 뽑으면 같은 길로 서면이 채워진다.")
A('')
A('---')
A('')
A('## 3-29. UI 그릇과 전투 바닥 — 생성만으로는 9-슬라이스가 안 된다 (2026-09-11)')
A('')
A('**잣대 둘을 새로 만들었다** — `measure_plate.py`(그릇) · `measure_bg.py`(바닥). 둘 다 리포에 둔다.')
A('')
A('**그릇 잣대.** 원본 64 · 모서리 16 에서 ① 가운데 32×32 의 휘도 표준편차 ② 네 변 조각을')
A('**늘어나는 축을 따라** 깊이 열여섯 줄로 갈라 각 줄의 표준편차. 변 조각은 코드가 늘리므로')
A('**한 줄이라도 흔들리면 그 줄만 띠가 된다** — 32 화소를 1408 화소로 늘리면 44 배로 벌어진다.')
A('')
A('**버튼 후보 16장 중 변이 평탄한 것은 0장이었다.** 사용자 승인본 c13 도 마찬가지다.')
A('')
A('| 대상 | 가운데 sd | 윗변 | 아랫변 | 왼변 | 오른변 |')
A('|---|---|---|---|---|---|')
A('| c13 원본 (승인본) | 0.000 | 0.39 | 3.31 | **12.03** | **8.70** |')
A('| **c13 고침** (설치본 `170e6f87`) | 0.000 | **0.00** | **0.00** | **0.00** | **0.00** |')
A('')
A('흔들리는 것은 **안쪽 밝은 빗각 선 하나**다. 나머지 열다섯 줄은 표준편차 0.000 이었다.')
A('1440 으로 늘리면 73% 지점에서 선이 한 단 밝아진 채 끝까지 갔다.')
A('')
A('**고친 방법** — `fix_nineslice.py`. 변 조각은 **늘어나는 축의 최빈 줄 하나**로 전부 채우고,')
A('가운데는 최빈색 하나로 채우고, **모서리 16 은 손대지 않는다.** 새 색은 한 개도 안 썼다')
A('(팔레트 20색 그대로). 바뀐 화소 **78 / 4096 = 1.9%**.')
A('사용자 지시 09-11 「9-슬라이스가 적용되도록 c13을 수정할 것 · 유사하기만 하면 됨」에 따른 것이다 —')
A('그 전까지는 「자르기·편집 아님」이 걸려 있어 재생성으로만 갔다.')
A('')
A('**문안 함정 하나 — 「가운데는 비어 있어야 한다」는 안 먹는다.**')
A('버튼 눌림 1차는 **16장 전량**이 빈 판 대신 칸 안에 물건을 그렸다(검·방패·물약·열쇠).')
A('가운데 평탄도 0.350~0.852 로 한 장도 비어 있지 않았다. 잠김 1차도 16 중 8 이 같았다.')
A('`mod_m` 의 X 와 같은 뿌리다(**규칙 9**) — 금지형 대신 **자리를 차지하는 것**을 적어야 한다:')
A('「바깥 16 픽셀 안쪽 전체가 한 가지 색 하나, 그 정사각형의 모든 화소가 동일한 색」.')
A('')
A('**바닥 잣대.** 평균 휘도(알파 문턱 16) · 상위 1% 채도 · 주황 화소 비율 ·')
A('**이음매**(끝 열과 첫 열의 평균 화소차)를 **이웃 열 평균 화소차와 나란히** 낸다 —')
A('이웃 값보다 작거나 같으면 이어 깔아도 줄이 안 보인다.')
A('')
A('| | 평균 휘도 | 최대 채도 | 주황 | 이음매 좌우 (이웃) | 상하 (이웃) |')
A('|---|---|---|---|---|---|')
A('| a_soil | 18.3% | 0.72 | 0.0% | 0.43 (3.07) | 0.28 (2.98) |')
A('| b_sand | 20.7% | 0.47 | 0.0% | 0.66 (0.64) | 0.49 (1.39) |')
A('| c_crack | 32.7% | 0.93 | 0.0% | 1.66 (3.89) | 5.33 (3.87) |')
A('| **d_grit** (사용자 확정 · 설치 `38ce9d2e`) | **28.2%** | 0.63 | 0.4% | 5.12 (5.49) | **7.19 (5.56)** |')
A('| 구판 `bg_combat` (`dc4d2238`) | 38.6% | 0.50 | 0.0% | **94.85 (8.10)** | 24.44 (7.20) |')
A('')
A('⚠️ **구판 `bg_combat` 은 애초에 이어지지 않았다** — 좌우 이음매 94.85 가 이웃 8.10 의 열두 배다.')
A('전투 화면에 세로줄이 있었다면 그 탓이며, 교체로 같이 해소된다.')
A('')
A('⚠️ **d_grit 은 넷 중 유일하게 상하 이음매가 이웃보다 높다**(7.19 vs 5.56). 세로로 여러 장 깔면')
A('가로줄이 보일 수 있다. 사용자 육안에서 안 걸렸으므로 그대로 두고 **참고값으로만 적는다.**')
A('')
A('**잣대 하나는 고쳐 썼다.** 처음에 「주황 화소 = 색상 10~45° · 채도 0.45 위」로 쟀더니')
A('바닥 후보 넷이 전부 **100%** 로 나왔다 — **갈색이 곧 어두운 주황 색상**이라 저 조건은 갈색을 전부 잡는다.')
A('**명도 0.55 위**를 더해야 강조 주황만 걸린다. 「측정 장치가 측정 대상을 만들지 않는지 먼저 본다」의 사례다.')
A('')
A('⚠️ **판정은 하지 않는다.** c13 의 색 축이 W02 6-1 과 다르다는 사실만 낸다 —')
A('채운 색 `#3B2B1B`(색상 30° · 채도 0.54) · 빗각 선 `#6D381F`(색상 19° · 채도 0.72) 로 **주황 축 안**이고,')
A('6-1 은 「반투명 검정 바탕 · 미색 테두리 · 주황은 코드 틴트」다. 어느 쪽을 고칠지는 설계 자리다.')
A('')
A('---')
A('')
A('## 3-30. 보스 바닥은 구판을 유지한다 (2026-09-11 · 사용자 판정)')
A('')
A('재생성 후보를 셋 냈고 **셋 다 상하 이음매가 잣대를 넘었다.**')
A('')
A('| 후보 | 휘도 | 좌우 이음매 (이웃) | 상하 이음매 (이웃) |')
A('|---|---|---|---|')
A('| `boss_dark` (`1c354788`) | 14.8% | 4.41 ≤ 4.69 ✅ | 5.32 > 4.74 ❌ 1.12배 |')
A('| `boss_e` (`72f0e608`) | 17.9% | 6.53 ≤ 6.65 ✅ | 9.28 > 6.71 ❌ 1.38배 |')
A('| `boss_f` (`0096cc6d`) | 18.9% | 4.97 ≤ 5.48 ✅ | 6.81 > 5.54 ❌ 1.23배 |')
A('| **구판 `b9a9da43`** | 20.6% | 7.52 ≤ 9.83 ✅ | **6.46 ≤ 7.22 ✅** |')
A('')
A('**「상하 이음매를 잡아라」를 문안으로 세 번 밀었고 세 번 다 안 됐다** — 2차는 「방향 없는 얼룩」,')
A('3차는 「고운 점묘」로 갔고 둘 다 「윗변 결이 아랫변 결의 연속이다」를 최우선에 두었다.')
A('좌우는 셋 다 통과하는데 상하만 걸린다. **변 평탄도와 같은 모양의 결론이다 —**')
A('**이음매는 문안으로 밀 자리가 아니라 공정으로 둘 자리다**(`art_to_plan` M-1-4).')
A('')
A('**다만 잣대를 통과·탈락으로만 쓰면 안 될 근거가 있다.** 이웃 값은 그 그림의 자연 잡음 높이이고,')
A('셋은 잡음의 **1.12~1.38배**인데 진짜로 깨진 구판 일반 바닥은 **11.7배**(94.85 / 8.10) 였다.')
A('한 자릿수가 아니라 **열 배 차이**다. 「≤ 이웃」 부등식보다 **배율 띠**가 맞다는 것이 아트 제안이며,')
A('어느 쪽이든 판정은 설계 자리다.')
A('')
A('**사용자 판정 — 구판 유지.** 아트가 「일반을 흙으로 바꿨더니 보스만 공장 판으로 남고,')
A('두 바닥의 색상이 32° 대 236° 로 정반대가 된다」를 사실로 올렸고, 사용자가 셋을 근거로 판정했다:')
A('**① 극명한 차이가 좋다 ② 보스 전투는 몬스터가 소량만 스폰한다 ③ 몬스터와 바닥의 구분에 문제가 없다.**')
A('')
A('⚠️ **부수 효과 하나** — 이 판정으로 `bg_combat_boss` 의 대장 승인 열이 **「기록 없음」에서 채워졌다.**')
A('배경 셋은 09-03 커밋 제목만 있고 육안 기록이 없던 자리였다(지침 §7 ［09-02］ 「커밋 제목을 승인으로 세지 않는다」).')
A('**`bg_board` 는 아직 비어 있다.**')
